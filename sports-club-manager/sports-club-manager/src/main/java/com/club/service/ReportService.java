...
public class ReportService {
    ...
    public List<Result> latestCompetitionRanking(){
        ...
        return resultRepo.all().stream().filter(new java.util.function.Predicate<Result>() {
                    @Override
                    public boolean test(Result r) {
                        return r.getCompetitionId().equals(cid);
                    }
                })
    ...
    public List<Map.Entry<String,Integer>> top5MembersAchievements(){
        ...
        return points.entrySet().stream()
                .sorted(new java.util.Comparator<Map.Entry<String, Integer>>() {
                    @Override
