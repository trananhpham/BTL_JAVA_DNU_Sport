...
public class ExportService {
    ...
    public void exportCompetitionReport(String competitionId, String outputPath) throws IOException {
        ...
        if (comp == null) throw new IllegalArgumentException("Giải đấu không tồn tại");
        ...
        try (PrintWriter pw = new PrintWriter(new OutputStreamWriter(
            ...
            List<Result> results = resultRepo.all().stream()
                    .filter(new java.util.function.Predicate<Result>() {
                        @Override
                        public boolean test(Result r) {
                            return r.getCompetitionId().equals(competitionId);
                        }
                    })
    ...
    public void exportAttendanceReport(String memberId, String outputPath) throws IOException {
        ...
        if (m == null) throw new IllegalArgumentException("Thành viên không tồn tại");
